...
from database import simpan_saldo, ambil_saldo  # Import fungsi dari database.py
from database import simpan_jurnal, ambil_jurnal  # pastikan ini sudah di-import
from database import ambil_saldo, ambil_jurnal
...
if "show_notification" not in st.session_state:
    st.session_state.show_notification = False  # or True, or whatever default you want

def show_notification():
    if st.session_state.show_notification:
        st.success("Notification: Something happened!")

show_notification()

# Autentikasi login
def login():
    st.sidebar.subheader("\U0001F510 Halaman Login")
...
# Initialize 'logged_in' if it doesn't exist
if "logged_in" not in st.session_state:
    st.session_state.logged_in = False

# Now it's safe to check the value
...
if menu == "Neraca Saldo":
    ...
    df_akun = [
        "Kas","Piutang Usaha", "Perlengkapan", "Persediaan Pakan", "Persediaan Madu", "Peralatan usaha", 
        "Kendaraan", "Akumulasi Penyusutan Kendaraan", "Utang Usaha", "Utang Gaji", "Utang Pajak", 
        "Modal, Alwi", "Prive, Alwi", "Ikhtisar Laba Rugi", "Penjualan", "Harga Pokok Penjualan", 
        "Beban Gaji Karyawan", "Beban Pakan Lebah", "Beban Perlengkapan", "Beban Perawatan Sarang", 
        "Beban Penyusutan Kendaraan", "Beban lain-lain", "Beban Pajak"
    ]
...
if menu == "Jurnal Umum":
    ...
    df_akun = ["Kas", "Piutang Usaha", "Perlengkapan", "Persediaan Pakan", "Persediaan Madu",
               "Peralatan usaha", "Kendaraan", "Akumulasi Penyusutan Kendaraan", "Utang Usaha",
               "Utang Gaji", "Utang Pajak", "Modal, Alwi", "Prive, Alwi", "Ikhtisar laba rugi",
               "Penjualan", "Harga Pokok Penjualan", "Beban Gaji Karyawan", "Beban Pakan Lebah",
               "Beban Perlengkapan", "Beban Perawatan Sarang", "Beban Penyusutan Kendaraan",
               "Beban lain-lain", "Beban Pajak"]
    ...
    with st.expander("Tambah Transaksi"):
        ...
        if st.button("Tambah Transaksi"):
            simpan_jurnal(str(tanggal), nama_akun, keterangan, saldo_debit, saldo_kredit)
...
if menu == "Buku Besar":
    ...
    saldo_awal = ambil_saldo().to_dict("records")
...
if menu == "Laporan Laba Rugi":
    ...
    # Ambil data neraca saldo disesuaikan dan saldo awal dari session_state
    ...
    if neraca_saldo:
        ...
        # Fungsi bantu ambil saldo debit/kredit sesuai nama akun
        ...
        # Ambil nilai dari neraca saldo disesuaikan
        ...
        # Ambil saldo awal persediaan madu dari saldo awal (neraca saldo awal)
        persediaan_madu_awal = get_saldo(df_awal, "Persediaan Madu")

        # Hitung biaya produksi dan HPP
        ...
        # Simpan laba bersih ke session_state agar bisa dipakai laporan lain
